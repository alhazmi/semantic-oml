...
sem_prepare(Database *db, DbTable* table)
{
  ...
  for (c=0; c<max; c++) {
	...
	if(osd && (strcmp(osd->verb,"MOFID:hasMetricAttributes") || strcmp(osd->verb,"MOFID:timestamp") || strcmp(osd->verb,"rdfs:label"))){
	osd = osd->next ;
  	}
	printf("ok3");
      if(osd){
	...
	while(strcmp(osd->verb,"MOFID:hasMetricAttributes") && strcmp(osd->verb,"MOFID:timestamp")){
      printf("ok while loop");
          osd = osd->next;
        }
	
	/*if(!strcmp(osd->verb,"MOFID:timestamp") && !strcmp(osd->predicate,"%value%")){
    printf("ok timestamp");
  pvar = sem_prepared_var(db,c);
  n += mstring_sprintf(insert,"\t\t?metric %s ?v%d .\n", 'MOFID:hasMetricAttributes', v);   
  n += mstring_sprintf(insert,"\t\t?v%d %s %s .\n", v, osd->verb, pvar);
   ...
   osd = osd->next;
	...
	//printf(mstring_buf(insert));
          ...
          logerror("%s: Failed in specification of the concepts, subject: %s doesnt have more info.\n",
            db->backend_name, first_concept_aux);
        	goto fail_exit;
      	}
...
sem_insert(Database *db, DbTable *table, int sender_id, int seq_no, double time_stamp, OmlValue *v, int value_count)
{
  ...
  if (semtable && semtable->insert_stmt) {
	...
	case OML_DATETIME_VALUE:
      ...
      /*if ((tok = strstr(old_tok, REPLACE_URI))) {
      	if(!strcmp(schema->fields[i].name,"hostname")){
      		*tok = '\0';
      		res += mstring_sprintf(stmtend, "%s\"%s%s\"", old_tok, "http://", omlc_get_string_ptr(*oml_value_get_value(v+i)));
        	old_tok = tok+strlen(REPLACE_URI);
      	} 
