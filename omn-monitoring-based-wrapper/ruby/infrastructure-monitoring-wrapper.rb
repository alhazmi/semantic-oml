		...
		total_memory = zbx.query(
			...
			cpu_load = zbx.query(
                        ...
                        unless cpu_load.empty?
                          cpu_load_ts = cpu_load[0]["lastclock"].to_i
                          cpu_load = cpu_load[0]["lastvalue"].to_f
                          CPU_LOAD_MP.inject(cpu_load, Time.at(cpu_load_ts).localtime.strftime("%Y-%m-%dT%H:%M:%S"), row[0], row[2])
                        end
